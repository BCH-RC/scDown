...
Users can load their scRNA-seq data which they would like to run the trajectory analysis. To run the trajectory analysis with `run_monocle3()`, the users have to provide the scRNA-seq data and species. Monocle3 requires raw gene expression values stored in the counts slot of the Seurat object. By default, the function runs the trajectory analysis on the whole dataset, identify the root nodes using potency method, and run trajectory analysis using monocle3. While `run_monocle3()` is generally species-agnostic and works beyond mouse or human, species specification is required when using `rootNodes_method = "potency"`. If you choose to manually define root nodes using `rootNodes_method = "rootNodes"`, then species specification is not necessary.
...
### Regression analysis between various groups in the scRNA-seq data
