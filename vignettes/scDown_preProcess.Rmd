...
Key Modules:
  ...
  4. scDown::run_scvelo: Incorporate spliced and unspliced counts and conduct RNA velocity analysis.
...
```{r, message=FALSE,eval=FALSE}
# Load your reference Seurat object
reference_seurat_obj_file <- "path/to/reference-Seurat-Object.rds"
reference_seurat_obj <- readRDS(reference_seurat_obj_file)
# Transfer cell type labels to your unannotated Seurat object
seurat_obj_annotated <- doTransferLabel(
...
```
The return `seurat_obj_annotated` will contain the predicted cell type labels from each cell and the confidence score of the label assignment and the updated UMAP coordinates (if  `transferCoordinates = TRUE`). This annotated Seurat object can now be used as input for scDown functions.
